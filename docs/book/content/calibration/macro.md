...
We set `world_int_rate_annual = 0.07`.  Per [OG-Core's open economy theory](https://pslmodels.github.io/OG-Core/content/theory/open_economy.html), the world interest rate $r^*$ is the rate the economy faces when accessing global capital markets; in the small-open limit it equals the domestic rate of return on capital, so $r^*$ must include the country risk premium.  Ethiopia's USD-denominated 10-year Eurobond carried a 6.625% coupon at 2014 issuance and traded at 8–14% yields before the December 2023 default, while non-concessional Chinese policy bank loans and post-float commercial trade financing typically run 6–9%.  We use 0.07 as the calibrated value, anchored on the Eurobond coupon; concessional terms (IDA, AfDB) are excluded because they are subsidized rather than marginal.  OG-Core's hard upper bound on this parameter is 0.08, which limits how aggressively the country-risk premium can be priced in.
